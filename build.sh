...
pacstrap -C ./mkimage-arch-pacman.conf -c -G -M -d "$buildfolder" filesystem shadow pacman gzip bzip2 sed grep gettext bash archlinux-keyring vi which
...
# init keyring
arch-chroot "$buildfolder" /bin/sh -c 'pacman-key --init; pacman-key --populate archlinux'

imageid=$(tar --numeric-owner -C "$buildfolder" -c . | docker import -)
docker tag $imageid voxxit/archlinux:$(date +%Y%m%d)
